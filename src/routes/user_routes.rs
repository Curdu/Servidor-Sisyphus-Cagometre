...
use crate::{controladors::user_controller::UserController, dades::models::rols::UsuariRol, errors::usuari_errors::UsuariErrors, routes::extractors::usuari_extractors::{ActualitzarUsuariRequest, CrearUsuariRequest, EliminarUsuariRequest, ModificarPerfilRequest, ObtenirUsuariPerIdRequest}, serveis::dtos::{auth_dto::AuthDataDTO, perfil_dto::PerfilDTO, usuari_dto::UsuariDTO}};
...
pub fn get_user_router(user_controller: Arc<dyn UserController>) -> Router{
    ...
    .route("/perfil/{id}", put(modificar_perfil))
...
async fn modificar_perfil
(
    State(user_controller) : State<Arc<dyn UserController>>,
    id: Path<Uuid>,
    Extension(claims): Extension<AuthDataDTO>,
    TypedMultipart(body) : TypedMultipart<ModificarPerfilRequest>
) -> Result<Json<PerfilDTO>, UsuariErrors> {
    let (imatge, perfil) = body.into();
    let nou_perfil = user_controller.modificar_perfil(*id, claims, perfil, imatge).await;
    match nou_perfil {
        Ok(perfil) => Ok(Json(perfil)),
        Err(error) => Err(error)
    }
}
